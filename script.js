...
function criaDiv(numberOfLines){
  for(let i = 0; i< numberOfLines; i++){
    let div = document.createElement('div');
    div.classList.add('line');
    document.querySelector('.triangle').appendChild(div);
  }
  
}

criaDiv(numberOfLines);
...
function fillTriangle(lines) {
  for(let index = 0; index < lines.length; index += 1) { //+1
...
function fillLine(divLine) {
  for (let lineColumn = 1; lineColumn <= basePyramid; lineColumn += 1){//+1 
    if(lineColumn >= controlLeft && lineColumn <= controlRight) {//+3
